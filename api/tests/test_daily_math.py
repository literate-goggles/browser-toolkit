...
def generated_algorithm_practice(*, valid_python: bool = True) -> dict:
    result = generated_practice("Algorithm")
    code = (
        "class Solution:\n"
    ...
    for problem in result["problems"]:
        problem["pythonSolution"] = code
        problem["followUp"]["pythonSolution"] = code
    return result


def leetcode_catalog() -> list[dict]:
    return [
        {
            "slug": "medium-one",
            "title": "Medium One",
            "difficulty": "Medium",
            "url": "https://leetcode.com/problems/medium-one/",
            "taskSummary": "Solve the first medium test problem using an efficient algorithm.",
            "functionSignature": "def mediumOne(self, nums: list[int]) -> int",
            "topics": ["array"],
        },
        {
            "slug": "medium-two",
            "title": "Medium Two",
            "difficulty": "Medium",
            "url": "https://leetcode.com/problems/medium-two/",
            "taskSummary": "Solve the second medium test problem using an efficient algorithm.",
            "functionSignature": "def mediumTwo(self, nums: list[int]) -> int",
            "topics": ["array"],
        },
        {
            "slug": "hard-one",
            "title": "Hard One",
            "difficulty": "Hard",
            "url": "https://leetcode.com/problems/hard-one/",
            "taskSummary": "Solve the hard test problem with a carefully justified algorithm.",
            "functionSignature": "def hardOne(self, nums: list[int]) -> int",
...
class DailyMathServiceTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_algorithm_day_mixes_book_medium_and_hard_with_python(self) -> None:
        ...
        self.manifest_file.write_text(
            json.dumps([algorithm_source]),
            encoding="utf-8",
        )
        (self.resources_dir / "algorithms.pdf.txt").write_text(
            "Chapter 1\nProblems\nA full algorithm source excerpt." * 4,
            encoding="utf-8",
        )
        self.manifest_file.with_name("leetcode_problems.json").write_text(
            json.dumps(leetcode_catalog()),
            encoding="utf-8",
        )
        self.generator = AsyncMock(return_value=generated_algorithm_practice())
        self.service = DailyMathService(
            data_file=self.data_file,
            manifest_file=self.manifest_file,
            resources_dir=self.resources_dir,
        ...
        problems = response.digest.subjects[0].problems
        self.assertEqual(
            [problem.sourceType for problem in problems],
            ["book", "leetcode", "leetcode"],
        )
        self.assertEqual(problems[1].sourceDifficulty, "Medium")
        self.assertEqual(problems[2].sourceDifficulty, "Hard")
        self.assertTrue(
            all(
                "class Solution" in problem.pythonSolution
                for problem in problems
            )
        )
        self.assertTrue(
            all(
                "class Solution" in problem.followUp.pythonSolution
                for problem in problems
            )
        )
        self.assertEqual(problems[1].title, problems[1].sourceTitle)
        persisted = json.loads(self.data_file.read_text(encoding="utf-8"))
        algorithm_history = persisted["history"]["algoday"]
        selected_medium_key = next(
            key
            for key in algorithm_history
            if key.startswith("leetcode:medium-")
        )
        self.assertTrue(selected_medium_key)
        self.assertIn("leetcode:hard-one", algorithm_history)
        next_selection = self.service._select_leetcode_problems(
            date(2026, 7, 25),
            algorithm_history,
        )
        self.assertNotEqual(
            f"leetcode:{next_selection[0].slug}",
            selected_medium_key,
        )
        prompt = self.generator.await_args.kwargs["messages"][1]["content"]
        self.assertIn("LeetCode Medium", prompt)
        self.assertIn("LeetCode Hard", prompt)

    async def test_leetcode_rotation_reuses_least_recent_problem(self) -> None:
        algorithm_source = source("algoday", "algorithms.pdf")
        self.manifest_file.write_text(
            json.dumps([algorithm_source]),
            encoding="utf-8",
        ...
        selection = self.service._select_leetcode_problems(
            date(2026, 8, 20),
            [
                "leetcode:medium-one",
                "leetcode:hard-one",
        ...
        self.assertEqual(selection[0].slug, "medium-one")
        self.assertEqual(selection[1].slug, "hard-one")

    async def test_algorithm_day_retries_invalid_python(self) -> None:
        algorithm_source = source("algoday", "algorithms.pdf")
        self.manifest_file.write_text(
            json.dumps([algorithm_source]),
            encoding="utf-8",
        ...
        self.generator = AsyncMock(
            side_effect=[
                generated_algorithm_practice(valid_python=False),
                generated_algorithm_practice(),
            ]
        )
        self.service = DailyMathService(
            data_file=self.data_file,
        ...
        self.assertEqual(self.generator.await_count, 2)
        self.assertTrue(response.digest.subjects[0].problems[0].pythonSolution)
